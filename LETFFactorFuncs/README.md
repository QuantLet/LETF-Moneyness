...
## [<img src="https://github.com/QuantLet/Styleguide-and-FAQ/blob/master/pictures/qloqo.png" alt="Visit QuantNet">](http://quantlet.de/) **LETFFactorFuncs** [<img src="https://github.com/QuantLet/Styleguide-and-FAQ/blob/master/pictures/QN2.png" width="60" alt="Visit QuantNet 2.0">](http://quantlet.de/)
...
Description : 'Compute and plot the estimated factor functions for the SPY ETF option data via the
dynamic semiparametric factor model. The third order is assumed, so 4 factor functions are
computed'
...
%% FACTOR FUNCTIONS' PLOT

figure
subplot(2,2,1)
surf(mongrid,ttmgrid,MHAT_FIN(:,:,1)')
hold on
xlabel('Moneyness')
ylabel('TTM')
title( '$$\hat{m}_0$$', 'Interpreter', 'Latex' )
hold off

subplot(2,2,2)
surf(mongrid,ttmgrid,MHAT_FIN(:,:,2)')
hold on
xlabel('Moneyness')
ylabel('TTM')
title( '$$\hat{m}_1$$', 'Interpreter', 'Latex' )
hold off

subplot(2,2,3)
surf(mongrid,ttmgrid,MHAT_FIN(:,:,3)')
hold on
xlabel('Moneyness')
ylabel('TTM')
title( '$$\hat{m}_2$$', 'Interpreter', 'Latex' )
hold off
